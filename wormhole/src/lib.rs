...
pub mod solana {
    ...
    pub fn send_msg(ctx:Context<SendMsg>, msg:String) -> Result<()> {
        //Look Up Fee
        let bridge_data:BridgeData = try_from_slice(&ctx.accounts.wormhole_config.data.borrow_mut())?;
    ...
    pub fn confirm_msg(ctx:Context<ConfirmMsg>) -> Result<()> {
        ...
        let vaa = try_from_slice(&ctx.accounts.core_bridge_vaa.data.borrow())?.0;
